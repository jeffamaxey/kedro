...
def test_run_got_dataframe(mocked_dataset):
    ...
    pattern = (
        r"Node.run\(\) expects a dictionary or None, "
        + r"but got <class \'kedro.io.lambda_dataset.LambdaDataSet\'> instead"
    )
...
class TestNodeRunInvalidInput:
    ...
    def test_one_input_error(self, mocked_dataset):
        ...
        pattern = (
            r"expected one input named 'ds1', but got the "
            + r"following 1 input\(s\) instead: \['arg'\]"
        )
        with pytest.raises(ValueError, match=pattern):
    ...
    def test_run_diff_size_lists(self, mocked_dataset):
        ...
        pattern = (
            r"expected 2 input\(s\) \['ds1', 'ds2'\], but "
            + r"got the following 1 input\(s\) instead."
        )
        with pytest.raises(ValueError, match=pattern):
    ...
    def test_run_diff_size_list_dict(self, mocked_dataset):
        ...
        pattern = (
            r"expected 1 input\(s\) \['ds1'\], but got the "
            + r"following 2 input\(s\) instead: \['ds1', 'ds2'\]\."
        )
        with pytest.raises(ValueError, match=pattern):
    ...
    def test_run_list_dict_unavailable(self, mocked_dataset):
        ...
        pattern = (
            r"expected 1 input\(s\) \['ds1'\], but got the "
            + r"following 1 input\(s\) instead: \['ds2'\]\."
        )
        with pytest.raises(ValueError, match=pattern):
...
class TestNodeRunInvalidOutput:
    def test_miss_matching_output_types(self, mocked_dataset):
        pattern = (
            r"The node output is a dictionary, whereas the function "
            + r"output is not\."
        )
        with pytest.raises(ValueError, match=pattern):
    ...
    def test_miss_matching_output_keys(self, mocked_dataset):
        pattern = (
            r"The node's output keys {'ret'} do not match "
            + r"with the returned output's keys"
        )
        with pytest.raises(ValueError, match=pattern):
            node(one_in_dict_out, "ds1", dict(ret="B", ans="C")).run(
    ...
    def test_node_not_list_output(self, mocked_dataset):
        pattern = (
            r"The node definition contains a list of outputs "
            + r"\['B', 'C'\], whereas the node function returned "
        )
